...
\begin{itemize}
	\item \textbf{NVOL}, a partition-aware objective function that minimizes the maximum per-partition outgoing communication volume rather than the global total. Chapters~\ref{cv_calculation} and~\ref{chapter5} describe its design.
	
	\item An implementation of NVOL within the METIS framework, integrated into the refinement stage with the supporting gain-table data structures. Chapter~\ref{chap:NVOL tooling} describes the adaptations needed to implement NVOL into METIS.
	
	%\item A simulated runtime model parameterized by per-partition incoming volume, message latency, and bandwidth, used to estimate the effect of partitioning choices on application runtime (Section~\ref{sec:model estimates}).
	
	\item An empirical study of NVOL against METIS's VOL objective across 20 graphs partitioned into 2 to 2048 parts, on both quality metrics and simulated runtime.
\end{itemize} 
%what was built 
...
\subsection{RQ2 - Do the partitions produced by NVOL improve performance?}

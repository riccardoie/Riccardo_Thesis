...
\newcommand{\auto}[1]{%
	\IfStrEq{#1}{N/A}{#1}{%
		\edef\a{\fpeval{abs(#1)}}%
		\ifnum\fpeval{(#1)<0}=1
		\ifnum\fpeval{\a>20}=1 \cellcolor{red!85}%
		\else\ifnum\fpeval{\a>5}=1 \cellcolor{red!65}%
		\else \cellcolor{red!45}\fi\fi
		#1%
		\else
		\ifnum\fpeval{\a>20}=1 \cellcolor{green!85}%
		\else\ifnum\fpeval{\a>10}=1 \cellcolor{green!65}%
		\else \cellcolor{green!45}\fi\fi
		\textbf{#1}%
		\fi
	}%
}

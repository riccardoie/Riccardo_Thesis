...
In Section~\ref{sec:greedy_refinement} of the previous chapter, we showed how the change in the objective function's value (the gain) is computed for the edge-cut objective function. This chapter extends that discussion to two other objective functions, both based on communication volume. The next chapter then shows how these gain calculations are used inside the greedy refinement algorithm.

The chapter is organized into four sections. The first introduces the basic definitions needed to understand communication volume operations. The second describes the gain calculation for the total communication volume objective function (VOL), which is more representative of actual communication cost than edge-cut, although it does not necessarily yield a well-balanced result. We treat VOL first because it is the standard approach used by partitioners such as METIS, and because it is easier to follow than NVOL on a first read, the gain for a vertex fits in a single vector.

The third section presents our novel gain calculation, used by NVOL, which provides finer information about how the volume of each individual partition is affected by a move. Here each vertex has a table representing the partition-specific gain of all possible moves. The section closes with an algorithm that ties the calculation together. The fourth and final section works through a few concrete examples to give the reader a clearer picture of how everything fits together.

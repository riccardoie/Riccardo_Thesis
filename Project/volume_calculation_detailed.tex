...
This chapter will show how the change in communication volume is calculated. This chapter will only focus on how gain is evaluated and implemented in the multilevel framework for communication volume. The next chapter will show how a vertex move is chosen using the results from this calculation. 

The total communication volume objective function (VOL) seeks to improve the total communication volume of a partitioned graph. Even though this metric is more representative of actual communication than edge cut, it does not necessarily produce a well balanced result. The net change in total communication volume after a potential move might hide bottlenecks and overloaded partitions. In contrast, NVOL delivers specific information about how the volume of each individual partition is affected by a move. Since NVOL was implemented on top of the METIS framework we will first spend some time formalizing how METIS and other partitioners perform calculations needed for VOL to work in Section~\ref{METIS gain calculation}. In Section~\ref{Partition specific gain calculation} we present the logic behind NVOL, and how we are able to evaluate how partitions are affected by a potential move.
...
This section extends the VOL gain calculation from Section~\ref{METIS gain calculation}. The VOL gain contains no information about which partition is affected by a move, information that is required to use NVOL during refinement. Here we show how to decompose the communication volume gain into its individual contributions to each partition. Once computed, this decomposition can be used to refine the graph; how NVOL uses it is discussed in Section~\ref{sec:NVOL refinement}.
...
\subsection{VOL gain implementation} \label{sec:global_gain_imp}
In Section~\ref{sec:preparation for refinement} the boundary set is computed using edge cut gain to filter and order it. When using VOL, we need to do the same thing using the calculations described in this chapter. Unlike the edge-cut metric, we cannot state for a given vertex whether moving it will lead to a positive gain or not. As stated in Section~\ref{sec:preparation for refinement}, we can deduce whether moving a vertex is guaranteed to give negative gain. This is not transferable to communication volume metrics.  
...
The implementation of NVOL's gain calculation required a different approach, even though the logic in the inner loop is very similar. Before starting refinement, METIS calculates and stores the gain of all boundary vertices. However, NVOL requires significantly more memory: for a gain vector of 10 elements, NVOL's gain table contains 90 entries. For this reason, we opted for an on-the-fly approach, computing the gain table for each vertex as it is visited during refinement. Beyond the memory overhead, a precomputed gain table would also require an update procedure to maintain consistency after each move. As this is not directly relevant to the research questions of this thesis, we leave this as future work, discussed further in Chapter~\ref{chap_future_work}.
...
\begin{algorithm}[h]
	\caption{ gain calculation}\label{alg:nvol}
	\begin{algorithmic}
		\State nparts $\gets k$ 							\Comment{Graph with $k$ partitions}
		\ForAll{$v \in V^{\mathrm{adj}}_i$}					\Comment{Visit all adjacent vertices of i}
		\For{n = 0; n < nparts}

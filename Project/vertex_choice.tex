...
If no candidate has been found yet, NVOL applies the relaxed first condition: destination $j$ is accepted as a candidate if tmpVol does not exceed the global $W^{\mathrm{max}}$, and the weight constraint $w(j) + w(i) \leq W^{\max} + f \cdot SG_i[\text{tmpPid}][j]$ is satisfied.
...
\begin{algorithm}[h]
	...
	\begin{algorithmic}
		\Require Vertex $i$ with neighbor partitions $N_i[0 \ldots n{-}1]$; Max partition volume $W^{\mathrm{max}}$
		\If{$w(P[i]) - w(i) \geq W^{\min}$}
		...
		\If{candidateVol = -1 \textbf{AND}\\
			\hspace*{4.1em}$W^{\mathrm{max}} \geq$ tmpVol \textbf{AND} \\
